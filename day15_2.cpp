...
int main (){
    ...
    for (i=1;i<=k;i++)
        d[i] = v[i]; /// facem o copie
    K = k;
    /// cautam binaaaaar
    st = 4, dr = 200;
    while (st <= dr){
        mid = (st+dr)/2;
        /// vedem daca reusim sa castigam cu puterea mid
        /// trebuie actualizat si b ul
        for (i=1;i<=n;i++)
            for (j=1;j<=m;j++)
                b[i][j] = 0;
        for (i=1;i<=K;i++){
        ...
        /// acum bagam aici codul de la punctul precedent
        ok = 0,pas = 0;
        for (;;){
            sort (v+1,v+k+1,cmp);
            for (i=1;i<=k;i++){
